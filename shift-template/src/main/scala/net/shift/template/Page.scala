...
object Snippet {
  ...
  def inline[T](name: String)(f: SnipState[T] => Try[(T, String)]) = new Inline(name, state[SnipState[T], String] {
    s => f(s) map { case (t, n) => (SnipState(PageState(t, s.state.lang, s.state.user), Nil, NodeSeq.Empty), n) }
  })
...
case class Inline[T](name: String, f: State[SnipState[T], String])
...
trait DynamicContent[T] {
  ...
  def inlinesMap: Map[String, State[SnipState[T], String]] = inlines.map((s: Inline[T]) => (s.name, s.f)).toMap
}
