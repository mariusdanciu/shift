...
object Binds {
  ...
  def bind(xml: NodeSeq)(bindFunc: PartialFunction[BNode, NodeSeq]): Try[NodeSeq] = {
    def _bind(xml: NodeSeq): NodeSeq = {
      xml flatMap {
        case Group(nodes) => _bind(nodes)
        case el: Elem =>
          val v = (applyPf(BNode(nameOf(el), Attributes(el.attributes), el.child))(bindFunc) getOrElse el)
          v match {
            case e: Elem =>
...
object HasClass {
  ...
  def unapply(a: BNode): Option[(String, Attributes)] = a.attributes.attrs.get("class") map (v => (v, a.attributes))
}

object HasClasses {
  def unapply(a: Attributes): Option[(List[String], Attributes)] = a.attrs.get("class") map (v => (v.trim.split("\\s+").toList, a))
  def unapply(a: BNode): Option[(List[String], Attributes)] = a.attributes.attrs.get("class") map (v => (v.trim.split("\\s+").toList, a.attributes))
}

object HasName {
  def unapply(a: Attributes): Option[(String, Attributes)] = a.attrs.get("name") map (v => (v, a))
  def unapply(a: BNode): Option[(String, Attributes)] = a.attributes.attrs.get("name") map (v => (v, a.attributes))
}

object HasValue {
  def unapply(a: Attributes): Option[(String, Attributes)] = a.attrs.get("value") map (v => (v, a))
  def unapply(a: BNode): Option[(String, Attributes)] = a.attributes.attrs.get("value") map (v => (v, a.attributes))
}

object HasId {
  def unapply(a: Attributes): Option[(String, Attributes)] = a.attrs.get("id") map (v => (v, a))
  def unapply(a: BNode): Option[(String, Attributes)] = a.attributes.attrs.get("id") map (v => (v, a.attributes))
}
